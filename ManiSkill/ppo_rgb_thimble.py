...
from thimble_env import Thimble, ColorObservationWrapper, InitialZeroActionWrapper, RenderStepInfoWrapper
...
class Args:
    ...
    """whether to save model into the `ManiSkill/runs/{run_name}` folder"""
    evaluate: bool = False
...
class NatureCNN(nn.Module):
    def __init__(self, sample_obs):
        ...
        # * THYMBLE ENV BEGIN
        # Process cup number information
        cup_feature_size = 64 
        extractors["cup_with_ball_number"] = nn.Sequential(
            nn.Linear(1, cup_feature_size),
            nn.ReLU()
        )
        self.out_features += cup_feature_size
        # * THYMBLE ENV END
...
if __name__ == "__main__":
    ...
    envs = ColorObservationWrapper(envs)
    envs = InitialZeroActionWrapper(envs, n_initial_steps=34)
    envs = RenderStepInfoWrapper(envs)
    ...
    if args.capture_video:
        eval_output_dir = f"ManiSkill/runs/{run_name}/videos"
        if args.evaluate:
            eval_output_dir = f"{os.path.dirname(args.checkpoint)}/test_videos"
        print(f"Saving eval videos to {eval_output_dir}")
        if args.save_train_video_freq is not None:
            save_video_trigger = lambda x : (x // args.num_steps) % args.save_train_video_freq == 0
            envs = RecordEpisode(envs, output_dir=f"ManiSkill/runs/{run_name}/train_videos", save_trajectory=False, save_video_trigger=save_video_trigger, max_steps_per_video=args.num_steps, video_fps=30)
    ...
    if not args.evaluate:
        ...
        writer = SummaryWriter(f"ManiSkill/runs/{run_name}")
        writer.add_text(
            "hyperparameters",
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if args.save_model and iteration % args.eval_freq == 1:
            model_path = f"ManiSkill/runs/{run_name}/ckpt_{iteration}.pt"
    ...
    if args.save_model and not args.evaluate:
        model_path = f"ManiSkill/runs/{run_name}/final_ckpt.pt"
        torch.save(agent.state_dict(), model_path)
        print(f"model saved to {model_path}")
