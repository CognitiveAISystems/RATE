...
@register_env("ShellGamePickBall-v2", max_episode_steps=90, asset_download_ids=["ycb"])
class ShellGamePickBall_v2(BaseEnv):
...
@register_env("ShellGamePick-v1", max_episode_steps=90, asset_download_ids=["ycb"])
class ShellGamePick(BaseEnv):
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            # ! GOAL
            goal_xyz = xyz.clone() + torch.tensor([0, 0, 0.2])
            goal_q = torch.tensor([0.707, 0, 0.707, 0]).repeat(b, 1)  # sqrt(2)/2 ≈ 0.707
            self.goal_site.set_pose(Pose.create_from_pq(p=goal_xyz, q=goal_q))
...
# env = gym.make("ShellGamePick-v1", num_envs=num_envs, obs_mode="rgb", n_initial_steps=50)
if __name__ == "__main__":
    num_envs = 4
    env = gym.make("ShellGamePick-v1", num_envs=num_envs, obs_mode="rgb")
...
# env = gym.make("ShellGamePick-v1", num_envs=num_envs, obs_mode="rgb", render_mode="rgb_array")
# env = ColorObservationWrapper(env)
# env = InitialZeroActionWrapper(env, n_initial_steps=30)
