...
class MemTransformerLM(nn.Module):
    ...
    def forward(self, states, actions, rtgs, target, timesteps, *mems, mem_tokens=None, masks=None):
        ...
        #     if self.env_name == 'mujoco' or self.env_name == 'maniskill-pushcube':
        ...
        #         self.loss_all = criterion_all(logits, target)
            ...
            # if self.env_name == 'associative-retrieval-seq-to-num':
            #     self.accuracy = 0
            #     self.loss_all = torch.tensor(0)
            #     self.loss_last = torch.tensor(0)
                
            # if self.env_name == 'vizdoom':
            #     loss = F.cross_entropy(logits.reshape(-1, logits.size(-1)),
            #                            target.reshape(-1).long())

            # if self.env_name == 'memory_maze':
            #     loss = F.cross_entropy(logits.reshape(-1, logits.size(-1)),
            #                            target.reshape(-1).long())
                
            # if self.env_name == 'minigrid_memory':
            #     loss = F.cross_entropy(logits.reshape(-1, logits.size(-1)),
