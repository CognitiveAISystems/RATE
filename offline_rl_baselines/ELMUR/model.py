...
from offline_rl_baselines.ELMUR.layers import (
...
class ELMURLayer(nn.Module):
    """A transformer layer with external memory that persists across time windows.

    This layer processes tokens in two parallel tracks:
    - Token processing: self-attention, then reading from memory, then feed-forward
    - Memory updates: writing from tokens, updating memory with LRU replacement policy

    The memory helps the model remember information across long sequences by maintaining
    a fixed-size memory bank that gets updated as new tokens are processed.
    """
    ...
    def __init__(self, 
        ...
        # Standard attention for memory interactions (tokens reading from memory)
        self.cross_attention1 = MultiHeadAttention(
            self.d_model, self.num_heads, dropout=dropout, attn_dropout=dropatt
        )
        # Standard attention for memory updates (memory learning from tokens)
    ...
    def forward(
        ...
        """
        Forward pass of a single ELMUR layer.
        ...
        # Process tokens through self-attention first
        if self.pos_encoding == 'relative':
            # For relative attention, don't pass memory states since ELMUR handles memory separately
        ...
        # Let tokens read from memory (if enabled)
        if self.use_tok2mem:
        ...
        # Update memory based on current tokens
        mem_res = mem_vec

        # Let memory learn from current tokens (if enabled)
        if self.use_mem2tok:
        ...
        # Return hidden representations (vocabulary projection done by ELMURModel)
        return h, new_memory_state, total_aux_loss


class ELMURModel(nn.Module):
    """ELMUR) model.

    A transformer-based model that uses external memory to handle long sequences
    efficiently. Each layer maintains its own memory bank that persists across
    time windows, allowing the model to remember important information over
    extended contexts.
    """
    ...
    def __init__(self,
        ...
        memory_init_std=0.02,    # How to initialize memory vectors
        detach_memory=True,       # Whether to detach memory between updates
        ...
        memory_dropout=None,     # Extra dropout for memory updates
        dtype="float32",         # Data type for model parameters
        sequence_format="sra",   # How to arrange tokens: s/state, r/rtg, a/action
        norm_type=None,  # Normalization type: "layer", "rmsnorm", or None (defaults to LayerNorm)
        # Mixture of Experts settings
        use_moe=False,           # Use multiple expert networks instead of single FFN
        num_experts=8,           # Total number of expert networks
        top_k=2,                 # How many experts to use per token
        expert_dropout=None,     # Dropout rate for expert selection
        load_balancing_loss_coef=0.01,  # Penalty for uneven expert usage
        use_swiglu=True,         # Use SwiGLU activation in experts

        # Advanced MoE options
        use_shared_expert=True,  # Include a shared expert that always activates
        n_shared_experts=1,      # Number of shared experts
        shared_d_ff=None,        # Feed-forward dimension for shared experts
        routed_d_ff=None,        # Feed-forward dimension for routed experts

        # Memory configuration
        use_shared_memory=False, # Share memory across all layers vs per-layer memory
        use_relative_bias=True,  # Use relative position bias in memory attention
        use_tok2mem=True,        # Enable tokens reading from memory
        use_mem2tok=True,        # Enable memory learning from tokens
        **kwargs
    ):
        super().__init__()
        ...
        # Set up positional encoding based on the chosen type
        if self.pos_type == "relative":
            ...
            # RoPE rotates queries and keys based on position
            self.pos_emb = RoPEPositionalEmbedding(self.d_head, max_seq_len)
        elif self.pos_type == "yarn":
            # YaRN improves RoPE for longer sequences
            L_train = max_seq_len      # Training length
            L_ext = max_seq_len * 4    # Target length (4x extension)
            m = min(128, self.d_head // 4) if self.d_head >= 32 else self.d_head // 2

            self.pos_emb = YaRNPositionalEmbedding(
                head_dim=self.d_head, L_train=L_train, L_ext=L_ext,
                base=10000.0, m=m, beta=32.0
            )
        elif self.pos_type == "alibi":
            # ALiBi adds distance-based penalties to attention scores
    ...
    def init_memory(self, batch_size: int, device: torch.device) -> List[MemoryState]:
        """Set up initial memory states for all layers.

        Returns either one shared memory state (used by all layers) or separate
        memory states for each layer depending on the configuration.
        """
    ...
    def forward(
        ...
        **kwargs
        ...
        """Run a forward pass through the ELMUR model.

        Takes in observations, actions, and targets, then processes them through
        the transformer layers with external memory. Returns action predictions
        along with updated memory states for the next time window.

        Args:
            states: Current state observations
            actions: Previous actions (for conditioning)
            rtgs: Return-to-go targets
            target: Target actions to predict
            timesteps: Time indices for this sequence
            memory_states: Memory from previous time windows
            pos_offset: Position offset for this sequence chunk

        Returns:
            Dictionary with predictions, updated memory, and auxiliary losses
        """
        ...
        # Set up absolute positions for this window
        ...
        output = {
            ...
            'mem_tokens': None,  # ELMUR doesn't use mem_tokens
