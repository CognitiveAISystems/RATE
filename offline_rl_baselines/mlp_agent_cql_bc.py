...
class DecisionMLP(TrajectoryModel):
    ...
    def forward(self, states, actions, returns_to_go, stacked_input=False):
        if self.mode == 'tmaze':
        ...
        mlp_outputs = self.mlp(stacked_inputs)

        if stacked_input:
            mlp_outputs = mlp_outputs[:,0::3,:]
        else:
            mlp_outputs = mlp_outputs

        action_preds = self.predict_action(mlp_outputs)

        state_action = torch.cat([mlp_outputs, actions], dim=-1)
        ...
        random_state_action = torch.cat([mlp_outputs, random_actions], dim=-1)
