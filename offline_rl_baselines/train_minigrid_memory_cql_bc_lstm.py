...
agent = DecisionLSTM(3, 4, 128, mode='minigrid_memory')
agent.train()
...
def create_args():
    ...
    parser.add_argument('--exp-name',       type=str, default='minigrid_memory', help='')
...
if __name__ == '__main__':
    ...
    run = wandb.init(project="RATE_MINIGRID_MEMORY_CQL", name=run_name, config=args, save_code=True)
    ...
    for epochs in tqdm(range(args.num_epochs), desc="Training epochs"):
        ...
        if epochs%10==0:
            PATH = f'offline_rl_baselines/ckpt/minigrid_memory_ckpt/{args.loss_mode}/{args.seed}/{run_name}'
