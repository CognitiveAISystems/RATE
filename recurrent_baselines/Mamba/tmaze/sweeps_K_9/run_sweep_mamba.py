...
with open("recurrent_baselines/Mamba/tmaze/sweeps_K_9/sweep_config.yaml") as f:
    sweep_config = yaml.load(f, Loader=yaml.FullLoader)

with open("recurrent_baselines/Mamba/tmaze/config_mamba_K_9.yaml") as f:
    config = yaml.load(f, Loader=yaml.FullLoader)

sweep_id = wandb.sweep(sweep_config, project=config['wandb_config']['project_name'])

def run_agent():
    subprocess.run(["python", "recurrent_baselines/Mamba/tmaze/sweeps_K_9/mamba_train_tmaze_sweep.py"])
