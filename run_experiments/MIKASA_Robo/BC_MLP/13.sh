...
python3 src/train.py \
    ...
    --wandb.wwandb=True \
    --data.gamma=1.0 \
    --data.path-to-dataset='data/MIKASA-Robo/unbatched/TakeItBack-v0' \
    ...
    --training.context-length=180 \
    --training.sections=1 \
    --model.env-name='mikasa_robo_TakeItBack-v0' \
    --model.state-dim=6 \
    --model.act-dim=8 \
    --model.d-model=64 \
    --model.dropout=0.0 \
    --model.padding-idx=None \
    --tensorboard-dir='runs/MIKASA_Robo/TakeItBack-v0' \
    --model-mode='BC' \
    --start-seed=1 \
    --end-seed=3 \
    --text='BC_MLP' \
    --online-inference.use-argmax=False \
    --online-inference.episode-timeout=180 \
    --online-inference.desired-return-1=180 \
