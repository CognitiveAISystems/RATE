    ...
    for k in range(steps):
        ...
        if isinstance(results, dict):
            ...
            if attn_map is None and hasattr(model, 'blocks') and len(model.blocks) > 0:
                ...
                attn_map = getattr(model.blocks[0].attn, '_attn_map', None)
        else:
            # MemTransformerLM returns tuple/list
            logits = results[0][:,-1,:]
            memory = results[1]
            mem_tokens = results[2] if len(results) > 2 else None
            attn_map = getattr(model, 'attn_map', None)
...
def get_returns_TMaze(model, ret, seed, episode_timeout, corridor_length, context_length, device, config, create_video=False):
    ...
    # Handle both model types for memory tokens
    if hasattr(model, 'mem_tokens') and model.mem_tokens is not None:
